...
from nfl_model.config_manager import NFLConfigManager, ConfigurationError
...
def test_load_basic_config():
    with tempfile.TemporaryDirectory() as td:
        ...
        mgr = NFLConfigManager(config_path=path)
...
def test_environment_overrides_merge():
    with tempfile.TemporaryDirectory() as td:
        ...
        path = write_yaml(td, cfg)
        mgr = NFLConfigManager(config_path=path, environment='production')
        loaded = mgr.load_config()
