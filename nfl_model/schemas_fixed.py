"""
Fixed NFL Spread Model schemas without circular references.
"""
...
class PowerRankingRecord(BaseModel):
    ...
    rank: int = Field(..., ge=1, le=32, description="Team rank")
    ...
    # Record information
    wins: Optional[int] = Field(None, ge=0, le=22, description="Wins")
    losses: Optional[int] = Field(None, ge=0, le=22, description="Losses")
...
class ScheduleRecord(BaseModel):
    ...
    date: Union[datetime, date, str] = Field(..., description="Game date")
    ...
    home_score: Optional[int] = Field(None, ge=0, le=100, description="Home team final score")
    away_score: Optional[int] = Field(None, ge=0, le=100, description="Away team final score")
    status: MatchupStatus = Field(default=MatchupStatus.SCHEDULED, description="Game status")
    
    @field_validator('date')
    @classmethod
    def validate_date(cls, v: Union[datetime, date, str]) -> datetime:
    ...
    def validate_matchup_logic(self) -> 'ScheduleRecord':
...
class ModelConfiguration(BaseModel):
    ...
    rmse_target: float = Field(default=10.0, gt=0.0, description="RMSE target")
