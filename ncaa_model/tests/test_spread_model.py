"""
Unit tests for NCAA Spread Model functionality.
"""
import pytest
from dataclasses import asdict

from ncaa_model.spread_model import SpreadCalculator, MatchupResult
...
class TestMatchupResult:
    ...
    def test_matchup_result_creation(self):
        ...
        result = MatchupResult(
            ...
            neutral_diff=10.5,
            home_field_adj=3.5,
            projected_spread=14.0,
            game_date="2024-10-15"
        )

        assert result.week == 5
        assert result.home_team == "Georgia Bulldogs"
        assert result.away_team == "Vanderbilt Commodores"
        assert result.home_power == 8.5
        assert result.away_power == -2.0
        assert result.neutral_diff == 10.5
        assert result.home_field_adj == 3.5
        assert result.projected_spread == 14.0
    ...
    def test_matchup_result_as_dict(self):
        ...
        result = MatchupResult(
            ...
            home_field_adj=3.0, projected_spread=5.0, game_date=""
        )
...
class TestSpreadCalculator:
    ...
    def custom_calculator(self):
        ...
        return SpreadCalculator(home_field_advantage=4.0)
    ...
    def test_calculator_initialization_default(self, calculator):
        ...
        assert calculator.home_field_advantage == 3.0
    ...
    def test_calculator_initialization_custom(self, custom_calculator):
        ...
        assert custom_calculator.home_field_advantage == 4.0
    ...
    def test_calculate_matchup_spread_home_favored(self, calculator):
        ...
        assert result.projected_spread == 15.0  # 12 + 3
        assert result.game_date == "2024-10-15"
    ...
    def test_calculate_matchup_spread_away_favored(self, calculator):
        ...
        assert result.projected_spread == -9.0  # -12 + 3
    ...
    def test_calculate_matchup_spread_pick_em(self, calculator):
        ...
        assert result.projected_spread == 1.0  # -2 + 3 (home field swing)
    ...
    def test_calculate_matchup_spread_custom_home_field(self, custom_calculator):
        ...
        assert result.home_field_adj == 4.0
        assert result.projected_spread == 4.0  # 0 + 4
    
    def test_calculate_week_spreads_success(self, calculator):
        """Test calculating spreads for multiple games in a week."""
        matchups = [
            ("Georgia Bulldogs", "Vanderbilt Commodores", "2024-10-15"),
            ("Alabama Crimson Tide", "Ole Miss Rebels", "2024-10-15"),
            ("Texas Longhorns", "Oklahoma Sooners", "2024-10-15")
        ]
        ...
        # Check first game: Bulldogs vs Commodores
        bulldogs_game = results[0]
        assert bulldogs_game.home_team == "Georgia Bulldogs"
        assert bulldogs_game.away_team == "Vanderbilt Commodores"
        assert bulldogs_game.projected_spread == 15.0  # (8 - (-4)) + 3

        # Check second game: Alabama vs Ole Miss
        tide_game = results[1]
        assert tide_game.home_team == "Alabama Crimson Tide"
        assert tide_game.projected_spread == 7.5  # (6.5 - 2) + 3

        # Check third game: Texas vs Oklahoma
        texas_game = results[2]
        assert texas_game.projected_spread == 7.5  # (3.5 - (-1)) + 3
...
class TestSpreadCalculatorEdgeCases:
    ...
    def test_extreme_power_rating_differences(self, calculator):
        ...
        assert result.projected_spread == 30.0  # 27 + 3
    ...
    def test_zero_power_ratings(self, calculator):
        ...
        assert result.neutral_diff == 0.0
        assert result.projected_spread == 3.0  # 0 + 3 (home field only)
    ...
    def test_precision_with_fractional_ratings(self, calculator):
        ...
        assert result.projected_spread == 10.0  # 7.0 + 3.0
    ...
    def test_matchup_result_with_empty_strings(self, calculator):
        ...
        assert result.projected_spread == 3.0
