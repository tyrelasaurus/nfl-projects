"""
Configuration Management System for NCAA Spread Model
...
from exceptions import NCAAModelError, ModelConfigurationError
...
class LoggingConfig:
    ...
    file: Dict[str, Any] = field(default_factory=lambda: {
        'enabled': False, 'filename': 'ncaa_model.log',
...
@dataclass
class NCAAModelConfig:
    """Complete NCAA model configuration."""
...
class NCAAConfigManager:
    """
    Manages loading and validation of NCAA model configurations.
    ...
    def __init__(self, config_path: Optional[str] = None, environment: Optional[str] = None):
        ...
        self.environment = environment or os.getenv('NCAA_MODEL_ENV', 'development')
        self._config: Optional[NCAAModelConfig] = None
    ...
    def load_config(self, reload: bool = False) -> NCAAModelConfig:
        """
        ...
        try:
            logger.info(f"Loading NCAA model configuration from: {self.config_path}")
            ...
            logger.info(f"NCAA model configuration loaded successfully for environment: {self.environment}")
            return self._config
            ...
            raise ConfigurationError(
                f"Failed to load NCAA model configuration: {str(e)}",
    ...
    def _build_typed_config(self) -> NCAAModelConfig:
        ...
        try:
            ...
            raise ConfigurationError(
                f"Failed to build typed NCAA model configuration: {str(e)}",
    ...
    def _validate_config(self) -> None:
        ...
        if errors:
            raise ConfigurationError(
                f"NCAA model configuration validation failed: {len(errors)} error(s)",
        ...
        logger.debug("NCAA model configuration validation passed")
    
    def get_config(self) -> NCAAModelConfig:
    ...
    def update_config(self, updates: Dict[str, Any]) -> None:
        ...
        logger.info("Updating NCAA model configuration with new values")
    ...
    def save_config(self, path: Optional[str] = None) -> None:
        ...
        try:
            ...
            logger.info(f"NCAA model configuration saved to: {output_path}")
            
        except Exception as e:
            raise ConfigurationError(
                f"Failed to save NCAA model configuration: {str(e)}",
                context={'output_path': output_path}
            )

# Global configuration manager instance
_ncaa_config_manager: Optional[NCAAConfigManager] = None

def get_ncaa_config_manager(config_path: Optional[str] = None, environment: Optional[str] = None) -> NCAAConfigManager:
    """Get or create the global NCAA configuration manager."""
    global _ncaa_config_manager
    
    if _ncaa_config_manager is None or config_path is not None:
        _ncaa_config_manager = NCAAConfigManager(config_path, environment)
    
    return _ncaa_config_manager

def get_ncaa_config() -> NCAAModelConfig:
    """Get the current NCAA model configuration."""
    return get_ncaa_config_manager().get_config()
