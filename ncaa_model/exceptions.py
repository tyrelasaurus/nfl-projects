"""
Custom exception classes for the NCAA Spread Model system.
...
class NCAAModelError(Exception):
    """Base exception for all NCAA Model system errors."""
    
    def __init__(self, message: str, context: Optional[Dict[str, Any]] = None, 
                 recovery_suggestions: Optional[List[str]] = None):
        """
        Initialize NCAAModelError.
...
class DataLoadingError(NCAAModelError):
...
class ScheduleLoadError(DataLoadingError):
    """Error loading NCAA schedule data."""
    ...
    def __init__(self, message: str, file_path: Optional[str] = None, 
        ...
        recovery_suggestions = [
            ...
            "Use NCAA API to download schedule if missing"
        ]
...
class SpreadCalculationError(NCAAModelError):
...
class ModelConfigurationError(NCAAModelError):
...
class OutputError(NCAAModelError):
...
class ValidationError(NCAAModelError):
...
class ModelPerformanceError(NCAAModelError):
...
class IntegrationError(NCAAModelError):
...
def handle_ncaa_model_exception(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        except NCAAModelError:
            # Re-raise our custom exceptions as-is
            raise
        except Exception as e:
            # Wrap unexpected exceptions
            raise NCAAModelError(
...
def log_model_error(error: Exception, logger: logging.Logger, 
    ...
    """
    Centralized error logging and handling for NCAA model.
    ...
    """
    try:
        if isinstance(error, NCAAModelError):
